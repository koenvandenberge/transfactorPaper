---
title: "Evaluate TF activity: repressions between TFs and targets: normalization"
...
```{r, eval=TRUE}
res <- evaluateSimulation_repressions_reprOffset(counts = countsAll,
                               design = design,
                               X = XAll,
...
for(nn in 1:nIter){
  ...
  saveRDS(dfAUCAll, file=paste0("../objects/dfAUCAll_dyngen",nn,"_benchmarkRepressionOffset.rds"))
}


date <- Sys.Date()
date <- gsub(x=date, pattern="-", replacement="")
saveRDS(dfAUCAll, file=paste0("../objects/dfAUCAll_dyngen",date,"_benchmarkRepressionOffset.rds"))
```

```{r}
dfAUCAll1 <- readRDS("../objects/dfAUCAll_dyngen1_benchmarkRepressionOffset.rds")
```
...
dfAUCAll$method <- factor(dfAUCAll$method, 
                          levels = c("poisson_lasso_repr", "poisson_lasso_repr_noOffset",
                                     "dirMult_lasso_repr", "dirMult_lasso_repr_noOffset",
                                      "viper", "AUCell"))
...
ggplot(dfAUCAll, aes(x=method, y=auc)) +
  ...
  xlab("") + ylab("AUC")
# ggsave("../plots/aucAllSettings_dyngen.pdf", width=12, height=9)
```
